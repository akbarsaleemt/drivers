/*THIS DRIVER CODE CAN ABLE TO BLOCK APPLICATION 
 *WHEN THERE IS NO DATA IN BUFFER AT THAT TIME WRITE 
 *APPLICATION WRITE DATA TO FILE. AFTER DATA WRITTEN
 *TO FILE WRITE DRIVER METHOD UNBLOCK READ METHOD
 *AND THEN READ APPLICATIONS READS THE DATA IN FILE
 */
...
static int my_start(){
  int ret;
  devno=MKDEV(MAJORN,0);
    ret=register_chrdev_region(devno,MINORN,DEVICE);
    if(ret<0){
        printk("<1>""device not registered");
        my_exit();
    }
    cdev_init(&mydev.mycdev,&fops);
    cdev_add(&mydev.mycdev,devno,MINORN);
    mydev.noofchar=0;
...
void my_exit(){
  ...
  unregister_chrdev_region(devno,MINORN);
